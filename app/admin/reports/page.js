...
export default function AdminReportsPage() {
  ...
  const fetchReport = async () => {
    ...
    try {
      const res = await axios.get(
        "http://localhost:4000/admin/reports/orders",
        {
          params: filters,
  ...
  // --- LOGIKA CHART (Mengelompokkan Data per Tanggal) ---
  const chartData = {
    labels: [],
    datasets: [
      {
  ...
  const downloadCSV = () => {
    ...
    const rows = orders.map((order) => [
      ...
      `"${order.user.name}"`, // Quote untuk handle koma di nama
      order.total,
      order.status,
      order.courier || "-",
    ...
    <>
      ...
      <div className="p-6 pt-[140px] max-w-7xl mx-auto">
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6 flex flex-wrap gap-4 items-end">
          ...
          <button
            onClick={downloadCSV}
            className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 text-sm h-10 ml-auto flex items-center gap-2"
          >
